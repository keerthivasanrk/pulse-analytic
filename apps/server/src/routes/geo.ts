import { FastifyInstance } from "fastify";
import { getGeoData } from "../db.js";

export default async function geoRoute(app: FastifyInstance) {
  app.get("/api/geo", async (request) => {
    const q = request.query as Record<string, string>;
    const filters = cleanFilters(q);
    return getGeoData(Object.keys(filters).length > 0 ? filters : undefined);
  });
}

function cleanFilters(q: Record<string, string>) {
  const f: Record<string, string> = {};
  for (const k of ["campaignId", "deviceType", "channel", "creativeType", "country"]) {
    if (q[k]) f[k] = q[k];
  }
  return f;
}
